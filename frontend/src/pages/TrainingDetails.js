...
const TrainingDetails = () => {
    ...
    const fetchTraining = async () => {
      try {
        const response = await fetch(
          `https://codecallbackend.vercel.app/training/${id}`
        );
        if (!response.ok) {
  ...
  useEffect(() => {
    if (training && training.repositoryLink) {
      const fetchRepoContents = async () => {
        try {
          const repoUrl = new URL(training.repositoryLink).pathname.substring(
            1
          );
  ...
  const File = ({ item }) => <div>📄 {item.name}</div>;

  const runCode = () => {
    const executePythonCode = async (code) => {
      try {
        const response = await fetch(
          "https://codecallbackend.vercel.app/execute-python",
          {
            method: "POST",
            headers: {
    ...
    <div className="training-details-wrapper">
        ...
        <div className="training-info-container">
          <div className="info-left">
            <div className="price">
              <div className="price-container">
                ...
                <div className="info-tooltip">
                  ...
                  >
                    <div className="reward-item feature">
                      {Math.round(training.points * 0.05)} XP Task 1
                    </div>
                    <div className="reward-item optimization">
                      {Math.round(training.points * 0.1)} XP Task 2
                    </div>
                    <div className="reward-item judging">
                      {Math.round(training.points * 0.15)} XP Task 3
                    </div>
                    <div className="reward-item bugs">
                      {Math.round(training.points * 0.2)} XP Task 4
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="tags">
              ...
              <span
                className={`tag difficulty ${training.difficulty.toLowerCase()}`}
              >
                {training.difficulty}
              </span>
            </div>
          ...
          <div className="submissions-container">
            {!username ? (
              ...
              <div className="submission-area">
                <p>
                  To complete each task add your code here and run it. You will
                  know if the task was completed when the task becomes crossed
                  out.
                </p>
                <CodeMirror
                  value={code}
                ...
                />
                <button onClick={runCode} className="styled-button">
                  Run Code
                </button>
                <pre>{output}</pre>
              </div>
      ...
      <div className="training-side-container">
        ...
        <div className="reward-card">
          <h3>Hints</h3>
          <div className="reward-container">{renderHints()}</div>
        </div>
      </div>
    </div>
  );

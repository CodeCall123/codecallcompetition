...
const CompetitionDetails = () => {
    ...
    const fetchCompetition = async () => {
      try {
        const response = await fetch(
          `https://codecallbackend.vercel.app/competitions/${id}`
        );
        if (!response.ok) {
  ...
  useEffect(() => {
    if (competition) {
      const intervalId = setInterval(() => {
        const { statusMessage, timeRemaining } = getRemainingTime(
    ...
    if (competition && accessToken) {
      ...
      const fetchUserPRs = async () => {
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Error fetching pull requests: ${response.statusText}`
            );
          }
          const data = await response.json();
  ...
  const renderRepoContents = (contents) => {
    return contents.map((item) => (
      <div key={item.sha} style={{ marginLeft: item.type === "dir" ? 20 : 0 }}>
        {item.type === "dir" ? <Folder item={item} /> : <File item={item} />}
      </div>
    ));
  };
  ...
  const File = ({ item }) => (
    <div>
      ...
      >
        <path
          d="M-6.11959e-07 6L-8.74228e-08 18C-3.93402e-08 19.1 0.9 20 2 20L14.01 20C15.11 20 16 19.1 16 18L16 2C16 0.899999 15.1 -6.60042e-07 14 -6.11959e-07L6 -2.62268e-07L-6.11959e-07 6ZM14.01 18L2 18L2 7L7 7L7 2L14.01 2L14.01 18Z"
          fill="white"
        />
      </svg>{" "}
      {item.name}
      ...
      <div key={pr.id} className="pr-container">
        ...
        {expandedPRs.includes(pr.number) && prDiffs[pr.number] && (
          <Diff
            ...
            diffType="unified"
            hunks={
              parseDiff(prDiffs[pr.number], { nearbySequences: "zip" })[0].hunks
            }
          >
            {(hunks) =>
  ...
  const renderJudges = () => {
    ...
    return (
      <div className="judges-card">
        <div className="judges-section">
          ...
          <p>
            {competition.judges.leadJudge ? (
              <a
                href={`https://github.com/${competition.judges.leadJudge.username}`}
                target="_blank"
            ...
            {competition.judges.judges.length > 0
              ? competition.judges.judges
                  .map((judge) => (
                    <a
                      key={judge._id}
                      href={`https://github.com/${judge.username}`}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      {judge.username}
        ...
        {!isJudge &&
          !isLeadJudge &&
          getRemainingTime(competition.startDate, competition.endDate)
            .statusMessage !== "ended" && (
            <button className="styled-button" onClick={handleBecomeJudge}>
              Become a Reviewer
            </button>
          )}
      </div>
  ...
  const handleAddJudge = async (type) => {
    try {
      const response = await fetch(
        `https://codecallbackend.vercel.app/competitions/${id}/addJudge`,
        {
          method: "POST",
          headers: {
  ...
  const handleBecomeJudge = async () => {
    try {
      const response = await fetch(
        `https://codecallbackend.vercel.app/competitions/${id}/becomeJudge`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="competition-content">
        <div className="competition-header">
          <div className="header-icons">
            <a
              href={competition.repositoryLink}
              target="_blank"
              rel="noopener noreferrer"
            >
              <FaGithub size={30} />
          ...
          <div className="header-content">
            ...
            <div className="live-status">
              ...
              >
                {getRemainingTime(competition.startDate, competition.endDate)
                  .statusMessage.charAt(0)
                  .toUpperCase() +
                  getRemainingTime(
                    competition.startDate,
                    competition.endDate
                  ).statusMessage.slice(1)}
              </span>
              {getRemainingTime(competition.startDate, competition.endDate)
                .statusMessage !== "ended" && (
          ...
          <div className="info-left">
            <div className="price">
              <div className="price-container">
                ...
                <div className="info-tooltip">
                  <strong>
                    Rewards for this contest comprise a pool broken down into
                  ...
                  >
                    <div>
                      <img src={usdcIcon} alt="USDC" className="usdc-icon" />{" "}
                      {Math.round(competition.reward * 0.5)} Feature
                    </div>
                    <div>
                      <img src={usdcIcon} alt="USDC" className="usdc-icon" />{" "}
                      {Math.round(competition.reward * 0.1)} Optimization
                    </div>
                    <div>
                      <img src={usdcIcon} alt="USDC" className="usdc-icon" />{" "}
                      {Math.round(competition.reward * 0.2)} Reviewing
                    </div>
                    <div>
                      <img src={usdcIcon} alt="USDC" className="usdc-icon" />{" "}
                      {Math.round(competition.reward * 0.2)} Bugs
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="tags">
              {competition.languages.map((lang, index) => (
                <span key={index} className={`tag ${lang.toLowerCase()}`}>
                  {lang}
                </span>
              ))}
              {competition.types.map((type, index) => (
                <span
                  key={index}
                  className={`tag ${type.toLowerCase().replace(/\s/g, "-")}`}
                >
                  {type}
                </span>
              ))}
        ...
        <div className="competition-main-content">
            ...
            {activeTab === "scope" && (
              <div>
                ...
                <p style={{ fontFamily: "Mulish" }}>
                  Below is the scope and files you need to work with, to view
                  the full scope, make sure to visit the projects' github page
                </p>
          ...
          {isLeadJudge && (
            <button
              className="styled-button"
              onClick={() => handleAddJudge("leadJudge")}
            >
              Add Lead Reviewer
            </button>
          )}
          ...
          <div className="submissions-container">
            ...
            {!username ? (
              ...
              <div className="submission-area">
                <p>
                  You are logged in. You can now push code to the repository and
                  your pull requests will be shown here.
                </p>
                <a
                  href={competition.repositoryLink}
                  target="_blank"
